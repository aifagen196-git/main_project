import axios from "axios";
import { withRunLog } from "./runLog.js";
import path from "node:path";
import { pathToFileURL } from "node:url";
import * as cheerio from "cheerio";
import dotenv from "dotenv";
import { createClient } from "@supabase/supabase-js";
import { normalizePersonio } from "./normalize/normalizePersonio.js";
import companies from "./config/personioCompanies.js";
import { isUsJob } from "./lib/isUsJob.js";

dotenv.config({ path: ".env" });

const supabase = createClient(
  process.env.SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY,
);

// =============================================
// CONFIG
// =============================================

const SCRAPE_LAST_HOURS = Number(process.env.SCRAPE_LAST_HOURS || 24);

const UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36";

// =============================================
// STATS
// =============================================

const stats = {
  companies: 0,
  fetched: 0,
  skipped: 0,
  processed: 0,
  saved: 0,
  failed: 0,
};

// =============================================
// SAVE JOB
// =============================================

async function saveJobs(jobs) {
  if (!jobs.length) return;
  const CHUNK = 500;
  for (let i = 0; i < jobs.length; i += CHUNK) {
    const batch = jobs.slice(i, i + CHUNK);
    const { error } = await supabase
      .from("jobs")
      .upsert(batch, { onConflict: "source,source_job_id" });
    if (error) {
      stats.failed += batch.length;
      console.error(`❌ batch of ${batch.length} failed: ${error.message}`);
      continue;
    }
    stats.saved += batch.length;
  }
}

// =============================================
// FETCH COMPANY JOBS
// =============================================

/**
 * Personio exposes a public, unauthenticated XML feed of open roles at
 * https://{company}.jobs.personio.de/xml (a handful of tenants live under
 * .com instead of .de -- tried as a fallback). The feed is a flat
 * <workzag-jobs><position>...</position></workzag-jobs> document, parsed
 * with cheerio in XML mode rather than pulling in a dedicated XML parser.
 */
async function fetchCompanyJobs(company) {
  for (const domain of ["jobs.personio.de", "jobs.personio.com"]) {
    try {
      const url = `https://${company}.${domain}/xml`;

      const { data } = await axios.get(url, {
        timeout: 15000,
        headers: { "User-Agent": UA },
      });

      if (typeof data !== "string" || !data.includes("<position>")) continue;

      const $ = cheerio.load(data, { xmlMode: true });

      return $("position")
        .map((_, el) => {
          const $pos = $(el);
          return {
            id: $pos.find("id").first().text().trim(),
            name: $pos.find("name").first().text().trim(),
            office: $pos.find("office").first().text().trim(),
            additionalOffices: $pos
              .find("additionalOffices > office")
              .map((__, o) => $(o).text().trim())
              .get(),
            employmentType: $pos.find("employmentType").first().text().trim(),
            createdAt: $pos.find("createdAt").first().text().trim(),
            descriptionHtml: $pos
              .find("jobDescriptions jobDescription value")
              .map((__, d) => $(d).text())
              .get()
              .join(" "),
          };
        })
        .get();
    } catch (err) {
      // try next domain
    }
  }
  console.error(`❌ Failed to fetch ${company}`);
  return [];
}

// =============================================
// MAIN
// =============================================

async function run() {
  console.log("\n==========================================");
  console.log("🚀 Personio Collector Started");
  console.log("==========================================\n");

  for (const company of companies) {
    try {
      stats.companies++;

      const jobs = await fetchCompanyJobs(company);

      stats.fetched += jobs.length;

      console.log(`\n🏢 ${company.toUpperCase()} (${jobs.length} jobs)`);

      const normalized = [];
      for (const job of jobs) {
        // Personio's XML feed only ever lists currently-open postings, same
        // as Breezy -- no publish-date cutoff needed, staleness handled by
        // the last_seen deactivation pass below.
        const normalizedJob = await normalizePersonio(job, company);
        if (!isUsJob(normalizedJob)) {
          stats.nonUs = (stats.nonUs || 0) + 1;
          continue;
        }
        normalized.push(normalizedJob);
        stats.processed++;
      }

      await saveJobs(normalized);
      if (normalized.length) console.log(`   💾 saved ${normalized.length}`);
    } catch (err) {
      console.error(`\n❌ Company failed: ${company}`);
      console.error(err.message);
    }
  }

  // =============================================
  // DEACTIVATE STALE JOBS
  // =============================================

  const staleCutoff = new Date(
    Date.now() - SCRAPE_LAST_HOURS * 60 * 60 * 1000,
  ).toISOString();

  const { error: deErr, count } = await supabase
    .from("jobs")
    .update({ is_active: false })
    .lt("last_seen", staleCutoff)
    .eq("source", "personio")
    .eq("is_active", true)
    .select("id", { count: "exact", head: true });

  if (deErr) {
    console.error(`⚠ Deactivation failed: ${deErr.message}`);
  } else {
    console.log(`🗑 Deactivated ${count ?? 0} stale jobs`);
  }

  console.log("\n==========================================");
  console.log("✅ Personio Collector Finished");
  console.log("==========================================");

  console.table(stats);

  console.log("==========================================\n");
}

// CLI-only guard — this file used to call run() unconditionally at
// import time, which meant `import()`-ing it for any reason (a test
// harness, a future registry, etc.) triggered a real scrape/save run as a
// side effect. Only run when invoked directly (node collectors/personio.js),
// matching every other collector in this repo (see runtime.js).
const entrypoint = process.argv[1];
if (entrypoint && import.meta.url === pathToFileURL(path.resolve(entrypoint)).href) {
  withRunLog("personio", run);
}
